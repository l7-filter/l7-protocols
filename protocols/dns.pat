...
# http://www.protocolinfo.org/wiki/DNS
...
# the length of the first part of the address, which is limited to 63 (0x3F).
# The next byte has to be a letter.  Then there can be an combination of 
# letters, digits, hyphens, and 0x01-0x3F.
...
^.?.?.?.?[\x01\x02].?.?.?.?.?.?[\x01-\x3F][a-z][\x01-\x3Fa-z]*[\x02-\x06][a-z][a-z][fglmoprstuvz]?[aeop]?(um)?[\x01-\x10][\x01\x03\x04\xFF]
